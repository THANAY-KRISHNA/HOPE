...
// --- FETCH REMOTE FLASHCARDS ---
async function fetchFlashcardData() {
    // USER CONFIG: Point this to your "Master" raw JSON file URL
    // This master file should look like: { "collections": ["url1", "url2", "path/to/local.json"] }
    const MASTER_MANIFEST_URL = 'data/flashcards_manifest.json';

    try {
        // 1. Fetch the Master Manifest
        const manifestRes = await fetch(MASTER_MANIFEST_URL);
        if (!manifestRes.ok) throw new Error(`Manifest fetch failed: ${manifestRes.status}`);

        const manifest = await manifestRes.json();
        const sources = manifest.collections || [];

        if (sources.length === 0) return;

        // 2. Fetch all referenced JSON files in parallel
        const deckPromises = sources.map(async (url) => {
            try {
                const r = await fetch(url);
                return await r.json();
            } catch (e) {
                console.warn(`Failed to load deck from ${url}`, e);
                return null; // Ignore failed decks
            }
        });

        const allDecks = await Promise.all(deckPromises);

        // 3. Aggregate Data
        let combinedCards = [];
        allDecks.forEach(deck => {
            if (deck && Array.isArray(deck.flashcards)) {
                combinedCards = combinedCards.concat(deck.flashcards);
            }
        });
        ...
        if (combinedCards.length > 0) {
            ...
            for (const k in flashcardData) delete flashcardData[k];
            ...
            let idBase = 1000;
            const thumbs = [
                "https://images.unsplash.com/photo-1544197150-b99a580bbcbf?auto=format&fit=crop&q=80&w=300",
            ...
            Object.keys(groups).forEach((topic, i) => {
                const deckId = idBase + i;
                flashcardSets.push({
                    id: deckId,
                    title: topic,
                    ...
                    <div class="flashcard-container" onclick="flipCard()">
                        <div class="flashcard ${state.flashcards.isFlipped ? 'flipped' : ''}" id="active-card">
                            <div class="card-face card-front">
                                <span class="card-label">Question ${card.subtopic ? `<span class="opacity-50 font-normal ml-2">(${card.subtopic})</span>` : ''}</span>
